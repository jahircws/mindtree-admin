                    		...
                    		<tbody>
                    			...
                    			if(!empty($states)){
                    				...
                    				foreach($states as $row){
                    					echo '<tr id="row_'.$row->state_id.'">';
                    					echo '<td>'.$i.'</td>';
                    					echo '<td>'.$row->state_title.'</td>';
                    					echo '<td>'.(($row->status)? '<div class="badge badge-success badge-shadow">Active</div>' : '<div class="badge badge-warning badge-shadow">Inactive</div>').'</td>';
                    					echo '<td><a href="javascript:toggleStatus('.$row->state_id.', '.(($row->status)? 'Deactive' : 'Active').');" class="btn btn-warning btn-sm ml-2" id="btn_status_'.$row->state_id.'">Status</a></td>';
                    					echo '</tr>';
